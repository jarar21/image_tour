...
def create_config(task_id, model, model_type, expected_repo_name, log_wandb=True):
    ...
    if log_wandb:
        config["wandb_runid"] = f"{task_id}_{expected_repo_name or 'run'}"
        config["wandb_name"] = f"{task_id}_{expected_repo_name or 'run'}"
        config["wandb_mode"] = "offline"  # Force offline mode
...
def patch_wandb_symlinks(base_dir: str):
    """Convert WandB symlink s into real files for offline use."""
...
async def main():
    ...
    print(f"WandB logs patched and stored at {train_cst.WANDB_LOGS_DIR}", flush=True)
